...
// Build headers for Votazioni (votes) sheet
$headers = [
    'Informazioni Giurati', 'Titolo', 'Categoria', 'Dove',
    'Scrittura','Regia','Scelta Cast / Tema','Coinvolgimento Emotivo','Senso di Novità','Casting / Artwork','Sonoro',
    'Totale','Media','Aggettivo','Giurato','Anno','Stagione','Episodio'
];
...
// Views
echo '<Worksheet ss:Name="Views ' . $exportYear . '">';
echo '<Table>';
echo '<Row>';
emit_cell('Platform','String','Header'); emit_cell('Category','String','Header'); emit_cell('Unique Titles','String','Header'); emit_cell('Views','String','Header'); emit_cell('Avg Rating','String','Header');
echo '</Row>';
...
// Judges (all)
echo '<Worksheet ss:Name="Judges ' . $exportYear . '">';
echo '<Table>';
echo '<Row>';
foreach (['Judge','Votes','Film','Serie','Miniseries','Documentario','Animazione','Avg Rating'] as $hc) emit_cell($hc,'String','Header');
echo '</Row>';
...
// Judges - Competition Only (similar but filtered by competition labels)
echo '<Worksheet ss:Name="Judges - Competition ' . $exportYear . '">';
echo '<Table>';
echo '<Row>';
foreach (['Judge','Votes','Avg Rating'] as $hc) emit_cell($hc,'String','Header');
echo '</Row>';
$sqlJudComp = "SELECT u.username AS judge, COUNT(v.id) AS votes, ROUND(AVG($ratingExpr),2) AS avg_rating FROM votes v JOIN users u ON u.id = v.user_id LEFT JOIN vote_details vd ON vd.vote_id = v.id WHERE (COALESCE(vd.competition_status,'') IN ('Concorso','In Competizione','In Competition')) AND " . $whereYear . " GROUP BY u.username ORDER BY votes DESC";
$judcomp = $mysqli->query($sqlJudComp)->fetch_all(MYSQLI_ASSOC);
foreach ($judcomp as $jc) { echo '<Row>'; emit_cell($jc['judge']); emit_cell((int)$jc['votes'],'Number'); emit_cell(isset($jc['avg_rating']) ? $jc['avg_rating'] : '','Number'); echo '</Row>'; }
echo '</Table>'; echo '</Worksheet>';

// Title List
echo '<Worksheet ss:Name="Title List ' . $exportYear . '">'; echo '<Table>'; echo '<Row>'; emit_cell('Title','String','Header'); echo '</Row>';
...
// Adjective List
echo '<Worksheet ss:Name="Adjective List ' . $exportYear . '">'; echo '<Table>'; echo '<Row>'; emit_cell('Title','String','Header'); emit_cell('Adjectives','String','Header'); echo '</Row>';
...
foreach ($rowsAdj as $a) { echo '<Row>'; emit_cell($a['title']); emit_cell($a['adjectives']); echo '</Row>'; }
echo '</Table>'; echo '</Worksheet>';

// Finalists
echo '<Worksheet ss:Name="Finalists ' . $exportYear . '">'; echo '<Table>'; echo '<Row>'; emit_cell('Title','String','Header'); emit_cell('Category','String','Header'); echo '</Row>';
...
// RAW (detailed votes) sheet: reuse headers (including extraVd) and safe emitting
echo '<Worksheet ss:Name="RAW ' . $exportYear . '">'; echo '<Table>'; echo '<Row>'; foreach ($headers as $h) emit_cell($h,'String','Header'); echo '</Row>';
foreach ($votes as $vote) {
    echo '<Row>';
    // Write the original header-mapped columns
    // Use the same mapping as Votazioni but with safe emit
    emit_cell($vote['username']); emit_cell($vote['title']); emit_cell($vote['category'] ?? ''); emit_cell($vote['where_watched'] ?? '');
    // numeric score columns (safe)
